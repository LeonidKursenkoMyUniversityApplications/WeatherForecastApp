...
namespace DAL.WeatherForecast.Controllers
{
    public class SqlQueryBuilder<T> where T : class
    {
        ...
        #region Find by Id
        public SqlCommand GetByIdCommand(int id)
        {
            var table = GetTableName();
            if (String.IsNullOrEmpty(table))
                throw new Exception("No Table attribute was found.");
        ...
        {
            var tableAttr = Attribute.GetCustomAttribute(typeof(T), typeof(TableAttribute));
